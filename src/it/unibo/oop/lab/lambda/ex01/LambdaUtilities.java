...
public final class LambdaUtilities {
	...
	public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
		...
		list.forEach(a -> {
			l.add(Optional.ofNullable(a).filter(pre)); // ofNullable si comporta in due modi: 1) se filter(pre) da null,
														// lui inserisce nella lista Optional.Empty
														// 2) se filter(pre) da qualcosa diverso da null, aggiunge
														// quell'elemento alla mia lista
		});
		return l;
	}
	...
	public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
		...
		list.forEach(a -> {
			m.merge(op.apply(a), new HashSet<>(Arrays.asList(a)), (PariDispari, elementi) -> { // Arrays.aslist gli
																								// passi dei valori e
																								// lui ti inizial9izza
																								// un array con quei
																								// valori e te li
																								// ritorna
				PariDispari.addAll(elementi);
				return PariDispari;
			});
